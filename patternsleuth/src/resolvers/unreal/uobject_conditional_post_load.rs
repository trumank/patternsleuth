...
fn match_postloading_assignments_tail(instructions: &Vec<Instruction>, local_index: &mut usize, matched_address: u64, try_match_tails_against_lea_register: Option<Register>, object_source_register: Register) -> bool {
    ...
    if *local_index + 1 < instructions.len() && instructions[*local_index].mnemonic() == Mnemonic::Mov && instructions[*local_index].op0_kind() == OpKind::Memory &&
        instructions[*local_index].op1_kind() == OpKind::Register && instructions[*local_index].op1_register() == object_source_register {
        ...
        *local_index += 1;
        ...
        if let Some(lea_storage_register) = try_match_tails_against_lea_register {
            ...
            if !is_volatile_register(async_package_register) && instructions[*local_index].mnemonic() == Mnemonic::Mov && instructions[*local_index].op0_kind() == OpKind::Memory &&
                instructions[*local_index].op1_kind() == OpKind::Register && instructions[*local_index].op1_register() == lea_storage_register &&
            ...
            if instructions[*local_index].mnemonic() == Mnemonic::Lea && instructions[*local_index].op0_kind() == OpKind::Register &&
                instructions[*local_index].op1_kind() == OpKind::Memory && instructions[*local_index].is_ip_rel_memory_operand() {
                ...
                if lea_instruction_address == matched_address && is_volatile_register(lea_destination_register) {
                    *local_index += 1;

                    // mov qword [rdi+0x98], lea_destination_register
                    if !is_volatile_register(async_package_register) && instructions[*local_index].mnemonic() == Mnemonic::Mov && instructions[*local_index].op0_kind() == OpKind::Memory &&
                        instructions[*local_index].op1_kind() == OpKind::Register && instructions[*local_index].op1_register() == lea_destination_register &&
                        instructions[*local_index].memory_base() == async_package_register {
                        return true;
                    }
                }
    ...
    // mov qword [rdi+0x98], $lea_storage_register
    if let Some(lea_storage_register) = try_match_tails_against_lea_register &&
        *local_index + 1 < instructions.len() && instructions[*local_index].mnemonic() == Mnemonic::Mov && instructions[*local_index].op0_kind() == OpKind::Memory &&
        instructions[*local_index].op1_kind() == OpKind::Register && instructions[*local_index].op1_register() == lea_storage_register {
        let async_package_register = instructions[*local_index].memory_base();
        *local_index += 1;
...
impl_resolver_singleton!(all, UObjectConditionalPostLoad, |ctx| async {
    ...
    #[derive(Copy, Clone, PartialEq, Eq)] enum V { Indirect, Direct }
    ...
    let patterns = strings.into_iter().flat_map(|(tag, _, str_addresses)| str_addresses.into_iter().flat_map(|str_addr| [
        format!("48 8d ?? X0x{str_addr:08x}"),
        format!("4c 8d ?? X0x{str_addr:08x}"),
    ].into_iter()).map(move |str| (tag.clone(), str))).collect_vec();
    let matched_addresses = join_all(patterns.into_iter().map(|(tag, str)| ctx.scan_tagged(tag, Pattern::new(str).unwrap()))).await.into_iter().collect_vec();
    ...
    for (pattern_tag, _, matched_addresses) in matched_addresses {
        for matched_address in matched_addresses {
            let maybe_function_start_address = ctx.image().get_root_function(matched_address)?.map(|f| f.range.start);
            if maybe_function_start_address.is_none() { continue }
            ...
            match pattern_tag {
                V::Indirect => {
                    ...
                    let bytes = ctx.image().memory.range(function_start_address..function_start_address + function_disassembly_length)?;
                    let instructions = Decoder::with_ip(64, bytes, function_start_address, DecoderOptions::NONE).iter().collect_vec();
                    ...
                    while index < instructions.len() {
                        ...
                        // mov rcx, rbx
                        if local_index + 1 < instructions.len() && instructions[local_index].mnemonic() == Mnemonic::Mov &&
                        instructions[local_index].op0_kind() == OpKind::Register && instructions[local_index].op0_register() == Register::RCX &&
                        instructions[local_index].op1_kind() == OpKind::Register {
                            ...
                            instructions[local_index].op0_kind() == OpKind::NearBranch64 && !is_volatile_register(object_source_register) {
                        ...
                        let mut try_match_tails_against_lea_register: Option<Register> = persistent_lea_storage_register;
                        ...
                        instructions[local_index].op1_kind() == OpKind::Memory && instructions[local_index].is_ip_rel_memory_operand() {
                            ...
                            if lea_instruction_address == matched_address {
                                ...
                                if is_volatile_register(lea_destination_register) {
                                    ...
                                    try_match_tails_against_lea_register = Some(lea_destination_register);
                                } else {
                                    // This is a lea with a persistent register, we want to match all tails against it from now on
                                    persistent_lea_storage_register = Some(lea_destination_register);
                                    try_match_tails_against_lea_register = persistent_lea_storage_register;
                                    index = local_index;
                                }
                            }
                        }
                        ...
                        if instructions[local_index].mnemonic() == Mnemonic::Mov && instructions[local_index].op0_kind() == OpKind::Memory {
                            ...
                            for candidate in candidates.iter().rev() {
                                ...
                                if match_postloading_assignments_tail(&instructions, &mut local_index_copy, matched_address, try_match_tails_against_lea_register, candidate.0) {
