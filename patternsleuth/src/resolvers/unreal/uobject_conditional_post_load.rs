...
fn match_postloading_assignments_tail(instructions: &Vec<Instruction>, local_index: &mut usize, lea_storage_register: Register, object_source_register: Register) -> bool {
...
impl_resolver_singleton!(all, UObjectConditionalPostLoad, |ctx| async {
    ...
    let matched_addresses = join_all(patterns.iter().map(|v| ctx.scan(Pattern::new(v).unwrap()))).await.into_iter().flatten().collect_vec();
    ...
    for matched_address in matched_addresses {
        ...
        while index < instructions.len() {
            ...
            // Try to match inline lea or persistent lea instruction
            ...
            if let Some(lea_register) = try_match_tails_against_lea_register && instructions[local_index].mnemonic() == Mnemonic::Mov &&
            instructions[local_index].op0_kind() == OpKind::Memory {
                ...
                for candidate in candidates.iter().rev() {
                    ...
                    if match_postloading_assignments_tail(&instructions, &mut local_index_copy, lea_register, candidate.0) {
