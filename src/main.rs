...
mod fns {
    ...
    pub fn functions(action: ActionFunctions) -> Result<(), Box<dyn Error>> {
        ...
        let symbols = dump_pdb(
            action.other_exe.with_extension("pdb"),
        ...
        let records = other_fn_dis
            ...
            .filter_map(|of| {
                ...
                if let Some(m) = m {
                    ...
                    //println!("radiff2 -g 0x{:x},0x{:x} {} {}", m.0.range.start, /*m.0.range.end,*/ of.0.range.start, /*of.0.range.end,*/ action.exe.display(), action.other_exe.display());
    ...
    pub fn sym(action: ActionSym) -> Result<(), Box<dyn Error>> {
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    match cli.action {
        ...
        Some(Action::Sym(action)) => return fns::sym(action),
        None => {}
    }
